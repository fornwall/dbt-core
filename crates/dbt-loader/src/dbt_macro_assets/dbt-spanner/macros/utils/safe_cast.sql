{# TODO: verify on Spanner. Spanner supports SAFE_CAST, UNNEST and ARRAY_AGG, but the
   BigQuery-specific STRUCT / GEOGRAPHY / JSON handling below may need adjustment. #}
-- funcsign: (string, string) -> string
{% macro spanner__safe_cast(field, type) %}
{%- if type.lower().startswith('array') and field is iterable and (field is not string and field is not mapping) and field | length > 0 -%}
    ...
    {#-- direct casts to arrays are not supported; each element must be cast individually + reaggregated into an array --#}
