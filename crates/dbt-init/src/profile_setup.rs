...
impl ProfileSetup {
    ...
    pub fn create_profile_for_adapter(
        ...
        let db_config = match adapter {
            ...
            AdapterType::Spanner => {
                let spanner_config = match existing_config {
                    Some(DbConfig::Spanner(config)) => Some(config),
                    _ => None,
                };
                DbConfig::Spanner(setup_spanner_profile(spanner_config.map(Box::as_ref))?)
            }
